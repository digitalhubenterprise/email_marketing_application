...
async def generate_groq_content(config: TelegramMarketingConfig, service: TelegramService, group_services: list[TelegramService]) -> str:
    ...
    system_prompt = (
        ...
        "-------------------------------------\n\n"
        "CRITICAL RULES:\n"
        "1. Format the headline strictly as: 🚀 [Service Title] | ( We Are Official Reseller / Distributor )([Group Name]) (replacing [Service Title] and [Group Name] with the actual service title and group name provided. Use 'Reseller' or 'Distributor' from context details/keywords, default to Reseller if not specified).\n"
        "2. Do NOT write any problem descriptions or long context paragraphs. The text must only contain the headline, the services list verbatim (divided by ------------------------------), the dynamic AI-generated short solution description paragraph, the dynamic urgency line (which you generate starting with 🔥), the registration line, the separator, and the footer.\n"
        ...
        "5. Generate a dynamic, high-conversion urgency call-to-action line starting with 🔥, tailored to the service (e.g. 🔥 Don't miss out on revenue - act now) directly below the short solution description paragraph.\n"
